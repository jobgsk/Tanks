...
namespace aicore
{
	...
	public:
		 ...
		 * violation will occur. A result of float _MAX indicates no violation will occur.
		 *
		 ...
		 * float _MAX.
		 */
		virtual float  willViolate(const Path* path, float  maxPriority) = 0;
	...
	public:
		...
		std::list<PhysicsBody*> obstacles;
		...
		virtual float  willViolate(const Path* path, float  maxPriority);
