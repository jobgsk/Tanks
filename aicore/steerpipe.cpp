...
namespace aicore 
{
	...
	void SteeringPipe::getSteering(SteeringOutput* output)
	{
		...
		Constraint *violatingConstraint = nullptr;
	...
	float AvoidSpheresConstraint::willViolate(const Path* path, float maxPriority)
	{
		float priority = REAL_MAX;
		float thisPriority;
		//std::list<Sphere*>::iterator soi;
		std::list<PhysicsBody*>::iterator soi;
	...
	float AvoidSpheresConstraint::willViolate(
		const Path* path, 
		float maxPriority, 
		//Sphere &obstacle
