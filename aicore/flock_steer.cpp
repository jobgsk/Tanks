	...
	float minDotProduct /* = -1.0 */)
{
	...
	for (bi = boids.begin(); bi != boids.end(); bi++, i++)
	{
		...
		{
			Vector3<float> offset = k->position - of->position;
...
Vector3<float> Flock::getNeighbourhoodCenter()
{
...
Vector3<float> Flock::getNeighbourhoodAverageVelocity()
{
	Vector3<float> center;
	std::list<aicore::Kinematic*>::iterator bi;
	unsigned i = 0;
...
void Separation::getSteering(aicore::SteeringOutput* output)
{
	...
	Vector3<float> cofm = theFlock->getNeighbourhoodCenter();
...
void VelocityMatchAndAlign::getSteering(aicore::SteeringOutput* output)
{
	...
	// Work out their center of mass
	Vector3<float> vel = theFlock->getNeighbourhoodAverageVelocity();
